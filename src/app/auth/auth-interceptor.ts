...
export class AuthInterceptor implements HttpInterceptor {
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler) {   // HttpRequest is of static type; intercept <any> outgoing requests
    const authToken = this.authService.getToken();  // calling token getter to retrieve
    const authRequest = req.clone({   // cloning is required; built-in method allows adding headers; 
